...
Free personal-team development signing (7-day expiry). `mobile/ota/ship-lan.sh` archives, signs, and exports `app.ipa`; install it with `xcrun devicectl device install app` over a USB cable or the same wifi network (#579 — the earlier Cloudflare/Tailscale tunnel install path was retired, nothing serves the IPA over the internet). `AppConfig.swift` reads `API_URL` env var if set, else defaults to `http://localhost:3000/api`.
