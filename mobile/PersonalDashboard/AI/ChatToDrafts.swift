...
struct ChatToDrafts {
    ...
    static func `default`() -> ChatToDrafts {
        ChatToDrafts(
            ...
            // The Shortcut's own executor, stamping `capture` on any meal it
            // writes (#546). `ExecuteDraftAction.default()` is the chat wiring
            // and would mislabel the provenance of a hands-free log.
            executor: ExecuteDraftAction(store: .shared, mealSource: MealSource.capture)
        )
    }
