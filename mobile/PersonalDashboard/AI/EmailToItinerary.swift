...
struct EmailToItinerary {
    ...
    private static func systemPrompt(timezone: String, nowIso: String, contextBlock: String) -> String {
        ...
        - Fields for add_expense:
          ...
          - category: EXACTLY one of these 13 values, nothing else — food_and_dining, groceries, transport, shopping, entertainment, bills_and_utilities, rent, health_and_wellness, travel, subscriptions, personal_care, gifts_and_donations, other. Pick the best fit (a flight/hotel fare → travel; a supermarket → groceries; Amazon goods → shopping; Netflix/Spotify → subscriptions; a utility bill → bills_and_utilities; a rent/lease payment → rent). Use "other" only when nothing fits.
