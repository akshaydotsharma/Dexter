...
/// Blocks and pagination for the trip expense report (#528).
///
...
///
/// The packing itself moved to `ReportPager` when the itinerary report needed
/// the same page and the same orphan rules (#532). The page geometry moved with
/// it: `ReportPageMetrics` now lives in `ReportPager.swift`.
///
...
enum ReportBlock: Identifiable, ReportPagedBlock {
    ...
    /// A day heading is the one run header the ledger has.
    var continuedRunHeader: ReportBlock? {
        if case .dayHeader(let title, let total, _) = self {
            return .dayHeader(title: title, total: total, continued: true)
        }
        return nil
    }

    /// The rows the day heading heads.
    var continuesOpenRun: Bool {
        if case .ledgerRow = self { return true }
        return false
    }
}
...
enum TripExpenseReportPaginator {
    ...
    /// Kept as the paginator's own entry point because the tests and the PDF
    /// renderer both call it; the algorithm itself is shared (#532).
