...
///   - Base widths echo the icon: ~50% (with a bullet), ~100% middle,
///     ~85% bottom.
...
///
/// Implementation note: `TimelineView(.animation)` is the only way to get
/// continuous motion inside a Live Activity — implicit `.animation` only
/// reacts to discrete state-driven changes, and `.repeatForever` modifiers
/// are explicitly NOT honoured by the system on activity surfaces. iOS
/// will still throttle / freeze the timeline when the activity is hidden
/// (e.g. the lock screen banner is collapsed), which is fine — the
/// expanded Dynamic Island and the live banner both run the timeline.
struct CaptureLogoLines: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 30.0, paused: phase != .processing)) { context in
        ...
        HStack(spacing: metrics.bulletGap) {
            ...
            if showBullet {
                Circle()
                    ...
                    .opacity(phase == .processing ? bulletOpacity(time: time) : 1)
            }
            Spacer(minLength: 0)
        }
    }
    ...
    private func lineWidth(
        ...
        let frequency = 1.6 // Hz, slow and calm — not a frantic loader.
        let amplitude: CGFloat = 0.13 // ±13% of the base width.
        let wave = sin(time * frequency * 2 * .pi + phaseOffset)
        let modulated = baseRatio + CGFloat(wave) * amplitude * baseRatio
        let clamped = min(max(modulated, 0.20), 1.0)
        return availableWidth * clamped
    }

    /// Bullet pulses opacity in sympathy with the lines. Runs at half the
    /// line frequency so it feels like a separate beat, not a metronome.
    private func bulletOpacity(time: TimeInterval) -> Double {
        let frequency = 0.8
        let wave = sin(time * frequency * 2 * .pi)
