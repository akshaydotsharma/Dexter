...
`~/.Codex/projects/-Users-akshaysharma-Documents-Projects-Dexter/memory/`
...
| New GitHub issue (bug / enhancement / feature) | orchestrator (direct `gh` CLI) | Title MUST follow `.Codex/rules/github-issue-conventions.md` — `[Bug]` / `[Enhancement]` / `[Feature]` prefix |
...
- **AI**: Anthropic `Codex-sonnet-4-5`, temperature 0.3, max 1024 tokens. Tool-use loop runs on-device; 15 tools in `mobile/PersonalDashboard/AI/ToolDefinitions.swift`. API key baked into IPA at archive time via `OTA_API_URL` / `ANTHROPIC_API_KEY` build settings.
...
Forward-looking rules live in `.Codex/rules/`. Read every file in that directory before doing project work.

- `.Codex/rules/github-issue-conventions.md` — issue title format and label conventions
- `.Codex/rules/dev-workflow.md` — local-first iOS workflow, tests-before-commit, commits link to issues
